...
; Enable UART ;переделано для ZX WIFI
...
/*     macro outp portU, value
	ld b, portU
	ld c, 0xEF ;основной порт ZX WIFI
    ld a, value
    out (c), a
    endm  */
	
; Internal port constants
port_uart	 = 0xEF ;основной порт ZX WIFI
...
;    push af
;    push hl
;    ld hl, is_recv ;если есть байт для приёма
;    ld a, 1
;    ld (hl), a
;    pop hl
;    pop af
...
;     ld a, (is_recv) ;если есть байт для приёма
;    and 1
;    jr nz, recvRet 
...
; recvRet: ;принять байт если мы знаем что он там есть
;    ld bc, ZXUNO_ADDR
;    ld a,  UART_DATA_REG
;    out (c),a
...
initWifi:
    call uartBegin
    ;call loadWiFiConfig ;отключена загрузка имени точки доступа
...
;     ld hl, cmd_cwjap1 ; Access Point connection ;отключена настройка на точку доступа
;    call uartWriteStringZ
;    ld hl, _ssid
;    call uartWriteStringZ
;    ld hl, cmd_cwjap2
...
startTcp_err:	
	call initWifi ;снова инициализация
	ld de,(port_tmp)
	ld hl,(addr_tmp)
	jp startTcp
port_tmp: defs 2
...
response_rdy:    defb "WIFI GOT IP", 0 ;было "ready"
